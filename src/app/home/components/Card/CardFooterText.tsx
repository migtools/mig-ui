...
const FooterText = ({ type, dataList, expandDetails }) => {
  const clusterText = dataList.length === 1 ? `View cluster` : `View clusters`;
  const storageText =
    dataList.length === 1 ? `View replication repository` : `View replication repositories`;
  const planText = dataList.length === 1 ? `View plan` : `View plans`;
  return (
